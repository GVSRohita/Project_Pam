...
var Location = function (idKey1, title_, cat_, locationLatLng_) {
    this.idKey = idKey1;
    this.title = title_;
    this.cat = cat_;
...
//as can be seen here.
var locations = [
    {idKey: 1, title: 'Kailash Giri', cat: 'Outing', location: {lat: 17.7492, lng: 83.3422}},
    {idKey: 2, title: 'Simhachalam', cat: 'Pilgrim Center', location: {lat: 17.7665, lng: 83.2506}},
    {idKey: 3, title: 'Indira Gandhi Zoological Park', cat: 'Outing', location: {lat: 17.7686, lng: 83.3445}},
    {idKey: 4, title: 'Visakha Museum', cat: 'Heritage', location: {lat: 17.7206, lng: 83.3342}},
    {idKey: 5, title: 'Sivaji Park', cat: 'Park', location: {lat: 17.7374, lng: 83.3312}},
    {idKey: 6, title: 'Rushikonda Beach', cat: 'Sun n Sand', location: {lat: 17.7820, lng: 83.3853}},
    {idKey: 7, title: 'MFC Restaurant', cat: 'Hotel', location: {lat: 17.7115, lng: 83.3195}},
    {idKey: 8, title: 'Hotel Daspalla', cat: 'Hotel', location: {lat: 17.7106556, lng: 83.3004312}},
    {idKey: 9, title: 'Vizag Steel Plant', cat: 'Industry', location: {lat: 17.6333889, lng: 83.1706543}}
];
...
var viewModal = function () {
    ...
    self.populatePlaceTitles = function () {
        ...
        for (var k = 0; k < locations.length; k++) {
            ...
            if (n >= 0) {
                ...
                //rectified issue in review obtained from first submission - begin
                markers[k].setMap(map);
            } else {
                //rectified issue in review obtained from first submission - end              
                markers[k].setMap(null);
            }
        }
        //Rectified after first submission
        //addMarkers();
    };
    ...
    self.showInfoWindow = function (chosenLoc) {
        ...
        var markerId = chosenLocation.idKey - 1;
    ...
    if ((address_ === undefined) || (title_ === undefined) || (category_ === undefined)) {
        ...
        geocoder.geocode({'address': address_}, function (results, status) {
            if (status === google.maps.GeocoderStatus.OK) {
                errorMessage('');
                var _idKey = myObservableArray().length + 1;
                var latitude = results[0].geometry.location.lat();
                var longitude = results[0].geometry.location.lng();
                var locationLatLng = new LocationLatLng(latitude, longitude);
                var location = new Location(_idKey, title_, category_, locationLatLng);
...
//Following functions created for rectifying the issues raised in the review
//report of first 
function highlightMarker(marker_) {
...
function googleError() {
    composeErrorMsg('Error in loading Google Map. Please mail this info "Error in loading Google Map" to gvsrohita@gmail.com to address this issue. Thanks!');
}